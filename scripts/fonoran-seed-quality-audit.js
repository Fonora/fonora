...
import { DIFFICULT_ONSETS } from '../tools/fonoran-phonetic-weights.js';
...
async function main() {
  ...
  const difficultRoots = new Set(
    Object.entries(rootSpelling).filter(([, sp]) => DIFFICULT_ONSETS.has(sp[0])).map(([id]) => id),
  );
  const dirtySeeds = [];
  ...
  await writeFile(outPath, JSON.stringify({ generated_at: new Date().toISOString(), phonetic_seed_pass: phoneticSeedPass, dirty_seeds: dirtySeeds, ...report }, null, 2) + '\n');
  console.log(`\nWrote ${outPath}`);

  process.exit(report.gate_pass && phoneticSeedPass ? 0 : 1);
}
